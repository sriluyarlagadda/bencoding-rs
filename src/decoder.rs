...
pub fn decode(input:&str) -> Result<BencodingResult, &str> {

	let mut peekable_input:Peekable<Chars> = input.chars().peekable();
...
pub fn decode_bytes(input:Vec<u8>) -> Result<BencodingResult, &'static str> {
	...
	return decode_peekable_bytes(&mut peekable_input)
}

fn decode_peekable_bytes(peekable_input:& mut Peekable<IntoIter<u8>>) -> Result<BencodingResult, &'static str> {
	let result:Result<Option<Vec<u8>>, &str> = decode_string_bytes(peekable_input);
	...
	let result:Result<Option<i64>, &str> = decode_int_bytes(peekable_input);
	...
	let result:Result<Option<Vec<BencodingResult>>, &str> = decode_list_bytes(peekable_input);
...
fn decode_peekable(peekable_input:& mut Peekable<Chars>) -> Result<BencodingResult, &'static str> {
	let result:Result<Option<String>, &str> = decode_string(peekable_input);
	...
	if let Ok(Some(decoded_string)) = result {
		return Ok(BencodingResult::Str(decoded_string))
	}
...
fn decode_string(peekable_input: &mut Peekable<Chars>) -> Result<Option<String>, &'static str> {

	let next_char:char = *(peekable_input.peek().unwrap());
	...
	let result_string = (peekable_input.take(count)).collect::<String>();
...
fn decode_int(peekable_input: &mut Peekable<Chars>) -> Result<Option<i64>, &'static str> {
	let next_char:char = *(peekable_input.peek().unwrap());
	if next_char != 'i' {
		return Ok(None)
	}
...
fn decode_map(peekable_input: &mut Peekable<Chars>) -> 
...
fn decode_string_bytes(peekable_input: &mut Peekable<IntoIter<u8>>) -> Result<Option<Vec<u8>>, &'static str> {
...
fn decode_int_bytes(peekable_input: &mut Peekable<IntoIter<u8>>) -> Result<Option<i64>, &'static str> {
	let next_char:char = *(peekable_input.peek().unwrap()) as char;
	if next_char != 'i' {
		return Ok(None)
	}
	...
	let error:Option<&'static str> = is_valid_format_bytes(&result_string);
...
fn is_valid_format_bytes(string: &str) -> Option<&'static str> {
	if string.is_empty() {
		return Some("Empty number")
	}
...
fn decode_list_bytes(peekable_input: &mut Peekable<IntoIter<u8>>) -> Result<Option<Vec<BencodingResult>>, &'static str> {
	if let Some(next_char) = peekable_input.peek() {
	...
	loop {
		...
		let next_value:Result<BencodingResult, &str>  = decode_peekable_bytes(peekable_input);
		if let Ok(result) = next_value {
			bencoded_list.push(result)
		} else {
			return Err(next_value.err().unwrap())
